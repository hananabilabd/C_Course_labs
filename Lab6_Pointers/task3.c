...
void display_Employee(employee emp[],int id_wanted ,int nOfDynamic){
    int found_ID ,found_flag=0,i;
     for ( i =0 ; i<nOfDynamic ;i++){
            if (emp[i].id == id_wanted){
                found_ID=i;
                found_flag = 1;
            }
        }
...
int main () {
    ...
    //employee emp[5] ;
	int mainMenu=1,display=0 , create=0,exit =0 ,x=0 ,flag =1,flag2=1,i,state = 0,flag_dynamic=0,n_dynamic;
	char c ;
	char  input_string[100];



        printf("Enter Number of Employees You wanna assign=");
        scanf("%d",&n_dynamic);
        employee *emp =(employee*) malloc (n_dynamic *sizeof(employee));
...
flag =0;
        ...
        scanf("%d",&id);
        //id =getche();
        display_Employee(emp ,id,n_dynamic);//Function to Search Employee Structure
   ...
   if (flag2 ==1){system("cls");}
    ...
    state=0;}
        ...
        system("cls");
        printf("\nYou Exceed Number of Employees\n");
        printf("Please Enter Anything \n");
    getch();
    mainMenu =1  ;
    flag =1;
